...
# saveRDS(models, "RDS_objects/MNLFA_models/cont_mods.RDS")

models <- readRDS("RDS_objects/MNLFA_models/cont_mods.RDS")
...
obtain_k <- function(model){
  est_k <- model$item %>% 
    filter(grepl("x",parm)) %>% # note: "x" is the name of the covariate
    select(est) 
  return(est_k)
}
...
saveRDS(results, "RDS_objects/Results/cont_res.RDS")
